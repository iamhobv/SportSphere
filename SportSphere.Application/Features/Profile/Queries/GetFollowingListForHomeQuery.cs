...
{
    public class GetFollowingListForHomeQuery : IRequest<ApiResponse<List<FollowerVm>>>
    {
        public string UserId { get; set; } = null!;
    }
    public class GetFollowingListForHomeValidator : AbstractValidator<GetFollowingListForHomeQuery>
    {
        public GetFollowingListForHomeValidator()
        {
            RuleFor(x => x.UserId).NotEmpty().WithMessage("UserId is required");
        }
    }
    public class GetFollowingListForHomeHandler : IRequestHandler<GetFollowingListForHomeQuery, ApiResponse<List<FollowerVm>>>
    {
        private readonly IGenericRepository<SocialProfile> profileRepo;
        private readonly IValidator<GetFollowingListForHomeQuery> _validator;
        private readonly IGenericRepository<Follow> followRepo;

        public GetFollowingListForHomeHandler(IGenericRepository<SocialProfile> profileRepo, IValidator<GetFollowingListForHomeQuery> validator
        ...
        public async Task<ApiResponse<List<FollowerVm>>> Handle(GetFollowingListForHomeQuery request, CancellationToken cancellationToken)
        {
            ...
            //var folowings = await followRepo.GetQueryable().AsNoTracking()
